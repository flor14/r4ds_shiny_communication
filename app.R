...
ui <- fluidPage(
    ...
    sidebarLayout(
        ...
        sidebarPanel(tags$h6("library(ggplot2)", style = "padding: 0px 0px; margin-top:0em"), br(), 
                     tags$h6("ggplot(mpg, aes(displ, hwy)) + ", style = "padding: 0px 0px; margin-top:0em"), br(), 
                     tags$h6("geom_point(aes(color = class)) +", style = "padding: 0px 0px; margin-top:-1em"), br(),
                     tags$h6("geom_smooth(se = FALSE) +", style = "padding: 0px 0px; margin-top:-1em"), br(),   
                     tags$h6("lab(", style = "padding: 0px 0px; margin-top:-1em"), br(),       
        ...
        mainPanel(
            h5("Proba cambiar las labels de este gráfico"),
    ...
    tabPanel("Annotations", 
             sidebarLayout(
                 div(style = "font-size: 10px; padding: 14px 0px; margin:0%",
              sidebarPanel(
                  tags$h6("best_in_class <- mpg %>%", style = "padding: 0px 0px; margin-top:0em"), br(),
                  tags$h6("group_by(class) %>%", style = "padding: 0px 0px; margin-top:-2em"), br(), 
                  tags$h6("filter(row_number(desc(hwy)) == 1)", style = "padding: 0px 0px; margin-top:-2em"), br(), 
                  tags$h6("ggplot(mpg, aes(displ, hwy)) +", style = "padding: 0px 0px; margin-top:-1em"), br(), 
                  tags$h6("geom_point(aes(colour = class)) +", style = "padding: 0px 0px; margin-top:-2em"), br(), 
                  selectInput("graph", "", choices = c("geom_text", "geom_label", "geom_label_repel"), selected = "geom_text"),
                  tags$h6("(aes(label = model), data = best_in_class, ", style = "padding: 0px 0px; margin-top:-0.5em"),
                  numericInput("nudge_x", "nudge_x =", value = 0),
                  numericInput("nudge_y", "nudge_y =", value = 2),
                  sliderInput("alpha", "alpha =", min = 0, max = 1, value = 0.5),
                  numericInput("size", "size =", value = 6),
                  numericInput("label_size", "label.size =", value = 0),
                 # checkboxInput("seg_col", "segment.color =", value = NA),
                  selectInput("hjust", "hjust = ", choices = c("top", "center", "bottom"), selected = "center"),
                  selectInput("vjust", "vjust = ", choices = c("left", "center", "right"), selected = "center")
                 ) #sidebarpanel
                 ), #div
              mainPanel( 
                h6("Añadiendo texto a mis puntos. ¿Qué tiene de especial la función ggrepel::geom_label_text?"),
             ...
             sidebarLayout(
               div(style = "font-size: 10px; padding: 14px 0px; margin:0%",
                   sidebarPanel(
                     ...
                     selectInput("legend_position", "legend.position =", choices = c("top","bottom", "right", "left", "none"), selected = "right"),
               ...
               mainPanel( 
                 h6("¿Qué ocurre si seteas el argumento label como NULL?  http://colorbrewer2.org/ /n breaks` and `labels` must have the same length"),
    ...
    tabPanel("Zooming",
             sidebarLayout(
               div(style = "font-size: 10px; padding: 14px 0px; margin:0%",
                   sidebarPanel(tags$h6("ggplot(mpg, aes(displ, hwy)) +", style = "padding: 0px 0px; margin-top:0em"), br(), 
                                tags$h6("geom_point(aes(color = class)) +", style = "padding: 0px 0px; margin-top:-1em"), br(),
                                tags$h6("geom_smooth(se = FALSE) +", style = "padding: 0px 0px; margin-top:-1em"), br(),   
               ...
               mainPanel(
                 h5("There are three ways to control the plot limits: 1. Adjusting what data are plotted
2. Setting the limits in each scale 3. Setting xlim and ylim in coord_cartesian()"),
    ...
    tabPanel("Theme",
             sidebarLayout(
                 ...
                 mainPanel(
                     h5("el paquete ggthemes te ofrece más posibilidades de temas"),
